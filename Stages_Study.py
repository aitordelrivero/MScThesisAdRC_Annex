...
def plotSweepEdited(fileName, title, results, sweepStart, sweepStop, sweepNum, sweepVar = 'auto', sweepScale = '', xVar = 'auto', xScale = '', xUnits = '', axisType = 'auto', funcType = 'auto', yVar = 'auto', yScale = '', yUnits = '', noiseSources = None, show = False):
    """
    Note: this function is almost equal as the original in https://github.com/Lenty/SLiCAP_python.
    It contains a few modifications to edit labels
    ...
    ax.traces = []
    if funcType == 'param':
        ###################### MODIFICATION TO EDIT VARIABLE NAMES ###############################
...
CpsDefaultBIG='5p'              #Pole splitting big capacitance  (from Script Freq. Compensation Study)
CpsDefaultSMALL='5p'            #Pole splitting small capacitance  (from Script Freq. Compensation Study)
...
Wout=5e-6                       #Output stage transistor width
Lout=350e-9                     #Output stage transistor length
IDout=5e-6                      #Output stage transistor drain current bias
Lcm=5e-6                        #1st stage Top biasing transistor length
Wcm=7.5e-6                      #1st stage Top biasing transistor width
Wb=5e-6                         #2nd stage biasing transistor width
Lb=1.5e-6                       #2nd stage biasing transistor length
Wbcm=8e-6                       #1st stage Bottom biasing transistor width
Lbcm=1.5e-6                     #1st stage  Bottom biasing transistor length



#VARIABLES COMING FROM CADENCE
CompensationArea=26.5e-6**2+(4+1)*350e-9*10e-6+1e-6*10e-6 #Cps+RpsPMOS+RpsNMOS+Rph1 areas
CparX=10.5e-15                                            #Parasitic at node X (cm in Cadence)
CparC=25.03e-15                                           #Parasitic at node C (out1 in Cadence)
CparY=29.73e-15                                           #Parasitic at node Y (net018 in Cadence)
...
#EXECUTION VARIABLES
circuit="FinalCircuit"          #Select circuit: SingleStage, TopBiasing1, BottomBiasing1,TwoStages, FreqComp_Both_NoBias, Biasing2, FinalCircuit.
ExtendedINFO=True               #Activates or deactivates extended info printing


#OTHER VARIABLES
ngtbc=3                         #Number of gains to be checked
GteChnNoiseFactor=9             #Gate resistance of a finger over the 1/gm of that finger (number of fingers estimation by Ravazi, must be between 5 and 10)


#LOADS CIRCUITS
Name=circuit
test=False
makeNetlist(Name+'.asc',Name)       #Creates netlist
i1=instruction()                    #Creates instruction object
i1.setCircuit(Name+'.cir')          #Loads circuit
print("-------------------")
print("CIRCUIT SET: "+Name)


AssignProcessParameters(i1)         #Assigns EKV process paramters
...
try: #Assigns parasitic extraction values
...
for gain in gains:                  #Assigns compensation parameters
    try:
    ...
    if ExtendedINFO:
        ...
        pz2html(loopgainpz)                 #loop gain poles and zeros (pole-zero compensation activated)
        i1.setDataType('poles')
        loopgainpoles=i1.execute()
        pz2html(loopgainpoles)              #loop gain poles (no pole-zero compensation)
        i1.setDataType('zeros')
        loopgainzeros=i1.execute()
        pz2html(loopgainzeros)              #loop gain zeros (no pole-zero compensation)
        i1.setGainType('servo')
        i1.setDataType('pz')
        servopz=i1.execute()
        pz2html(servopz)                    #servo function poles and zeros to study stability
...
##############################################################################
##############################################################################
############# PLOTS TRANSFERS ASYMPTOIC GAIN MODEL ###########################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
...
f_check=10e6                                #frquency max_limit of the graphs
i1.setSimType('numeric')                    #numeric simulation to substitute all variables with their values
i1.setSource('I1')                          #the source is the membrane current source
i1.setDetector('V_out')                     #the detector is the output (input of the ADC)
i1.setLGref(reference)                      #reference source in the asymptotic model
i1.setStepVar('gain')                       #transfers for ngtbc gains between max_gain and min_gain
i1.setStepMethod('log')
i1.setStepStart(min_gain)
i1.setStepStop(max_gain)
i1.setStepNum(ngtbc)
i1.stepOn()
#obtains the transfers
i1.setGainType('loopgain')
...
#plots the transfers in Bode plots (magnitude and phase)
ini.plotFontSize=18
...
##############################################################################
##############################################################################
############# TOP BIASING TRADE-OFFS #########################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################

if Name=="TopBiasing1": #Trade-offs and performance parameters for the 1st stage top biasing
    ini.defaultColors=['#c3312f','#00b7d3','#99d28c','#f1be3e','#007188','k','#eb7245','#0066a2','#00a390','#82d7c6']
    #Noise performance
    i1.defPar('NEF', '1+(g_m_XU3/(g_m_XU1))') #Noise Excess Factor
    #Gain performance
    i1.defPar('Av_stg1', '(g_m_XU1)*(1/((g_o_XU1)+g_o_XU3))*(2*g_m_XU3*(1/g_o_XU3)+1)/(2*(g_m_XU3*(1/g_o_XU3)+1))') #Estimated gain of the first stage with a current mirror as top biaisng
    i1.defPar('Av_stg1_woCM', '((g_m_XU1)/2)*(1/((g_o_XU1)+g_o_XU3))') #Estimated gain of the first stage considering independent top biasing (two separated branches instead of a current mirror)
    i1.defPar('Av_stg1_woCMideal', '(g_m_XU1)/(g_o_XU1)') #Estimated gain of the first stage with ideal biasing
    i1.defPar('GEFcm', '(Av_stg1)/Av_stg1_woCMideal') #"Gain excess factor", unused
    #Noise-Gain performance
    i1.defPar('GONcm', 'GEFcm/NEF') #Gain excess factor over Noise excess factor, unused
    #Area performance
    i1.defPar('AreaCM', '2*Wcm*Lcm') #Area ocuppied by the top biasing transistors (payload)
    i1.defPar('AreaBCS', '2*Was*L')  #Area ocuppied by the bottom biasing transistors (payload)
    i1.defPar('AREAfactor', '(AreaCM+AreaBCS)/(AreaBCS)') #"Area excess factor", unused
    #Noise-gain-area performance
    i1.defPar('GONAcm', 'GEFcm/(NEF*AREAfactor)') #unused
    #Input voltage swing performance
    i1.defPar('Vsat_XU1','(2*U_T*sqrt(IC_XU1+0.25)+3*U_T)/2')  #Transistors input stage estimated saturation voltage
    i1.defPar('Vsat_XU2','2*U_T*sqrt(IC_XU2+0.25)+3*U_T')      #Left transistor top biasing input stage estimated saturation voltage
    i1.defPar('Vsat_XU3','2*U_T*sqrt(IC_XU3+0.25)+3*U_T')      #Right transistor top biasing input stage estimated saturation voltage
    i1.defPar('MaxIN_woCM', 'Vdd-Vsat_XU3-Vsat_XU1+V_GS_XU1')  #Estimated maximum input voltage without a curret mirror as top biasing
    i1.defPar('MaxIN', 'Vdd+V_GS_XU3-Vsat_XU1+V_GS_XU1')       #Estimated maximum input voltage with a curret mirror as top biasing
    i1.defPar('MaxINratio', 'MaxIN/MaxIN_woCM')                #Voltage swing loss ratio (because of using the current mirror)
    #PSRR
    i1.defPar('PSRR','g_m_XU1/(g_o_XU1+g_o_XU3)')              #Estimated Power Supply Rejection Ratio for low frequencies, referred to the clamping voltage (PSRRv@DC)

    params2html(i1.circuit)

    #Study the influence of W (stepping) and L (sweeping) of the top biasing transistors in the performance
    i1.setDataType('params')
    i1.stepOn()
    i1.setStepVar('Wcm')   #Stepping W
    ...
    Sweep = plotSweepEdited('WCurrentMirror1', '$Speed: f_T$', result, 350e-9, 10e-6, 50, axisType = 'semilogy', sweepVar= 'Lcm', xUnits = 'm', yVar = 'f_T_XU3', yUnits = 'Hz', funcType = 'param', show = False) #Sweeping L
    fig2html(Sweep, 600)
    #Noise trade-off
    head2html('Adjusting Wcm based on noise requirement')
    Sweep2 = plotSweepEdited('WCurrentMirror2', 'Noise: NEF', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lcm', xUnits = 'm', yVar = 'NEF', yUnits = 'Hz', funcType = 'param', show = False) #Sweeping L
    fig2html(Sweep2, 600)
    #Isolation trade-off
    head2html('PSRR')
    Sweep8 = plotSweepEdited('WCurrentMirror8', '$Isolation: PSRR$', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lcm', xUnits = 'm', yVar = 'PSRR', yUnits = '', funcType = 'param', show = False) #Sweeping L
    fig2html(Sweep8, 600)
    #Swing trade-off
    head2html('MaxIN')
    Sweep9 = plotSweepEdited('WCurrentMirror9', '$Swing: Vin_{max}$', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lcm', xUnits = 'm', yVar = 'MaxIN', yUnits = 'V', funcType = 'param', show = False) #Sweeping L
    fig2html(Sweep9, 600)
...
if Name=="BottomBiasing1":
    ...
    #CMRR
    i1.defPar('CMRR', '(1+2*(g_m_XU1)*(1/g_o_XU6))*g_m_XU3*(1/(g_o_XU1+g_o_XU3))') #Estimated common-mode rejection ratio
    #SWoptg
    i1.defPar('Vsat_XU1','(2*U_T*sqrt(IC_XU1+0.25)+3*U_T)/2')  #Transistors input stage estimated saturation voltage
    i1.defPar('Vsat_XU2','2*U_T*sqrt(IC_XU2+0.25)+3*U_T')      #Left transistor top biasing input stage estimated saturation voltage
    i1.defPar('Vsat_XU3','2*U_T*sqrt(IC_XU3+0.25)+3*U_T')      #Right transistor top biasing input stage estimated saturation voltage
    i1.defPar('Vsat_XU6','2*U_T*sqrt(IC_XU6+0.25)+3*U_T')      #Transistor bottom biasing input stage estimated saturation voltage
    i1.defPar('VIN_min', 'V_GS_XU1+Vsat_XU6')                  #Estimated minimum input voltage
    #Area
    i1.defPar('AreaBcm', 'Wbcm*Lbcm') #Estimated area occupied by the first stage bottom biasing transistors (payload)

    #Study the influence of W (stepping) and L (sweeping) of the bottom biasing transistors in the performance
    params2html(i1.circuit)
    i1.setDataType('params')
    i1.stepOn()
    i1.setStepVar('Wbcm') #Stepping W
    i1.setStepStart(500e-9)
    i1.setStepStop(30e-6)
    i1.setStepNum(10)
    i1.setStepMethod('log')
    result = i1.execute()
    Sweep2 =  plotSweepEdited('SizingBias2', '$Swing: Vin_{min}$', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lbcm', xUnits = 'm', yVar = 'VIN_min', yUnits = 'V', funcType = 'param', show = False) #Sweeping L
    fig2html(Sweep2, 600)
...
if Name=="TwoStages" or Name=="FreqComp_Both_NoBias":
        head2html('Trade-offs')
        i1.defPar('Av_stg2ideal', 'g_m_XU4*(1/(g_o_XU4))')      #Estimated ideal gain second stage
        i1.defPar('Vsat_XU4','2*U_T*sqrt(IC_XU4+0.25)+3*U_T')   #Transistor output stage estimated saturation voltage
        i1.defPar('Vout_max', 'Vdd-Vsat_XU4')                   #Estimated maximum output voltage
        i1.defPar('AreaO', 'Wout*Lout')                         #Ouput transistor area usage (payload)
        params2html(i1.circuit)
        #Study the influence of W (stepping) and L (sweeping) of the second stage transistor in the performance
        i1.setDataType('params')
        i1.stepOn()
        i1.setStepVar('Wout') #Stepping W
        i1.setStepStart(500e-9)
        i1.setStepStop(60e-6)
        i1.setStepNum(10)
        i1.setStepMethod('log')
        result = i1.execute()
        Sweep2 = plotSweepEdited('SizingOut2', '$Swing: Vout_{max}$', result, 350e-9, 30e-6, 50, axisType = 'lin', sweepVar= 'Lout', xUnits = 'm', yVar = 'Vout_max', yUnits = 'V', funcType = 'param', show = False) #Sweeping L
        fig2html(Sweep2, 600)
        """
        Sweep1 = plotSweep('SizingOut1', 'Gain of the second stage', result, 350e-9, 30e-6, 50, axisType = 'lin', sweepVar= 'Lout', xUnits = 'm', yVar = 'Av_stg2ideal', yUnits = '', funcType = 'param', show = False) #Sweeping L
        fig2html(Sweep1, 600)
        """
        i1.stepOff()
...
##############################################################################
##############################################################################
############# OUTPUT STAGE BIASING TRADE-OFFS ################################
################  OR FINAL CIRCUIT SUMMARY  ##################################
##############################################################################
##############################################################################
##############################################################################
...
if (Name=="Biasing2" or Name=="FinalCircuit" or Name=="ParasiticCM" or Name=="ParasiticNET108" or Name=="ParasiticOUT1" or Name=="AllParasitics"):
        head2html('Trade-offs')
        i1.defPar('Av_stg2ideal', 'g_m_XU4*(1/(g_o_XU4))')              #Estimated ideal gain second stage
        i1.defPar('Av_stg2', 'g_m_XU4*(1/(g_o_XU4+g_o_XU5))')           #Estimated gain second stage considering biasing
        i1.defPar('GEF_out', 'Av_stg2/Av_stg2ideal')                    #unused: "gain excess factor"
        i1.defPar('Vout_max', 'Vdd-Vsat_XU4')                           #Estimated maximum output voltage
        i1.defPar('Vout_min', 'Vsat_XU5')                               #Estimated minimum output voltage
        i1.defPar('AreaB', 'Wb*Lb')                                     #Area used by the ouput stage biasing (payload)
        i1.defPar('Vov_XU1', 'V_GS_XU1-Vth_N18')                        #Overdrive voltage estimation (square-law)
        ...
        i1.defPar('VeffSI_XU1','IDas/g_m_XU1')                          #Conservative estimation of the saturation voltage (Jeespers-Murmann)
        ...
        i1.defPar('Vsat_XU1','(2*U_T*sqrt(IC_XU1+0.25)+3*U_T)/2')       #Estimation of the saturation voltage (Binkley)
        ...
        #Study the influence of W (stepping) and L (sweeping) of the second stage biasing transistors in the performance
        params2html(i1.circuit)
        i1.setDataType('params')
        i1.stepOn()
        i1.setStepVar('Wb') #Stepping W
        i1.setStepStart(500e-9)
        i1.setStepStop(60e-6)
        i1.setStepNum(10)
        i1.setStepMethod('log')
        result = i1.execute()
        Sweep1 = plotSweep('SizingBias1', '$Avstg2(L_{b})$', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lb', xUnits = 'm', yVar = 'GEF_out', yUnits = '', funcType = 'param', show = False) #Sweeping L
        fig2html(Sweep1, 600)
        Sweep2 = plotSweepEdited('SizingBias2', '$Swing: Vout_{min}$', result, 350e-9, 10e-6, 50, axisType = 'lin', sweepVar= 'Lb', xUnits = 'm', yVar = 'Vout_min', yUnits = 'V', funcType = 'param', show = False) #Sweeping L
        fig2html(Sweep2, 600)
        i1.stepOff()

        #Clean parameters for the summary
        ...
        #Create summary table with the W,L,ID, IC, gm, to and suggested number of fingers for each tranistor in the controller
        ...
        #Print summary table with the W,L,ID, IC, gm, to and suggested number of fingers for each tranistor in the controller
        fig, ax = plt.subplots()
        ax.set_axis_off()
        table = ax.table(
        cellText = datatable,
        ...
        cellLoc ='center',
        loc ='upper left')
        ax.set_title('Transistor sizing', fontweight ="bold")
        fig.savefig('./img/TransistorSizingTable.svg',dpi=600)
        img2html('TransistorSizingTable.svg',600)

        #Print summary list with OpAmp performance parameters (estimated noise, input and ouput ranges, gains, area, bandwidth, power, PSRR...)
        ...
        servoData = findServoBandwidth(loopgain1.laplace)      #calculate the frequency cut off and the midband value of the loopgain
        f_h = servoData['lpf']
        text2html('OpAmp BW (worst case max_gain): '+str(sp.N(f_h,3))+' Hz')

        i1.defPar('gain', m_gain)
        loopgain2 = i1.execute()
        servoData = findServoBandwidth(loopgain2.laplace)      #calculate the frequency cut off and the midband value of the loopgain
        f_h = servoData['lpf']
        text2html('OpAmp BW (m_gain): '+str(sp.N(f_h,3))+' Hz')

        i1.defPar('gain', min_gain)
        loopgain3 = i1.execute()
        servoData = findServoBandwidth(loopgain3.laplace)      #calculate the frequency cut off and the midband value of the loopgain
        f_h = servoData['lpf']
        text2html('OpAmp BW (min_gain): '+str(sp.N(f_h,3))+' Hz')

        #PLOT BODE OF ALL ASYMPTOTIC GAIN TRANSFERS FOR GAIN 20GV/A
        ini.plotFontSize=18
        ...
        #PLOT BODE OF ALL ASYMPTOTIC GAIN TRANSFERS FOR GAIN 200MV/A
        ini.plotFontSize=18
        ...
        #PLOT BODE OF ALL ASYMPTOTIC GAIN TRANSFERS FOR GAIN 2MV/A
        ini.plotFontSize=18
